...
These fixtures are imported by valuation, CLI, and domain tests.
"""
...
# ---------------------------------------------------------------------------
# Minimal dataclass-backed namespace builders
# These duck-type the live domain objects without importing their builders.
# They keep fixture values audit-stable while supporting dataclasses.replace()
# in valuation tests that mutate selected fields.
# ---------------------------------------------------------------------------

_FIXTURE_CLASS_CACHE: dict[tuple[str, ...], type] = {}


def _ns(**kwargs):
    field_names = tuple(kwargs)
    cls = _FIXTURE_CLASS_CACHE.get(field_names)
    if cls is None:
        cls = make_dataclass(
            "FixtureObject",
            [(name, Any) for name in field_names],
        )
        _FIXTURE_CLASS_CACHE[field_names] = cls
    return cls(**kwargs)
...
def make_orcl_metrics():
    ...
    valuation = _ns(
        ...
        normalized_fcf=16_648_000_000.0,   # normalized FCF handling: positive normalised FCF
        capex_spike_detected=True,          # normalized FCF handling: capex spike flag
    )
    ...
    ratios = _ns(
        ...
        return_on_equity=0.4211,           # ROE cap handling: exceeds technology cap of 0.35
...
def make_adbe_metrics():
    ...
    cash_flow = _ns(
        ...
        dividends_paid_ttm=0.0,            # buyback distribution handling: zero dividends
        share_buybacks_ttm=-10_509_000_000.0,  # buyback distribution handling: large buybacks
    ...
    ratios = _ns(
        ...
        peg_ratio=47.7871,
        peg_growth_source="ttm_ni_growth",
        return_on_equity=0.6305,           # ROE cap handling: exceeds technology cap of 0.35
...
def make_ai_metrics():
    financials = _ns(
        ...
        net_income_ttm=-434_502_000.0,      # NEGATIVE — negative-income growth filtering trigger
        revenue_ttm_prev=310_582_000.0,
        net_income_ttm_prev=-279_696_000.0,
        da_ttm=13_606_000.0,
        revenue_growth_rate=-0.0103,
        net_income_growth=0.5535,           # negative-income growth filtering: ratio of two negatives
        gross_margin=0.4345,
    ...
    cash_flow = _ns(
        ...
        fcf_ttm=-127_047_000.0,            # normalized FCF handling trigger: dual-negative
        last_year_fcf=-44_446_000.0,       # normalized FCF handling trigger: dual-negative
    ...
    valuation = _ns(
        ...
        normalized_fcf=None,               # normalized FCF handling: no normalised FCF for AI
        capex_spike_detected=False,
    )
