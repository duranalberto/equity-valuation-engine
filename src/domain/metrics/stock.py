...
class Valuation:
    ...
    raises ``dataclasses.FrozenInstanceError`` immediately, making accidental
    mutation visible as an error rather than a silent wrong value.

    Construction
    ------------
    Never construct directly.  Use the ``build()`` classmethod:

        valuation = Valuation.build(
            financials=..., balance_sheet=..., market_data=...,
            cash_flow=..., historical_data=...,
            highest_price=..., cost_of_debt=..., corporate_tax_rate=...,
        )

    The ``highest_price``, ``cost_of_debt``, and ``corporate_tax_rate``
    parameters are loader-supplied seeds; everything else is derived inside
    ``build()``.
    """
    ...
    def build(
        ...
        *,
        ...
        Loader-derived inputs
        ---------------------
        ``cost_of_debt`` and ``corporate_tax_rate`` were previously derived
        inside ``StockMetrics._compute_derived_valuation_inputs``.  They are
        now computed here so all logic lives in one place:
...
class StockMetrics:
    ...
    ``__post_init__`` now delegates all computation to the two immutable
    factory methods.  The construction order is explicit in the factory
    signatures rather than hidden in a chain of private methods.

    Phase order
    -----------
    1. ``Valuation.build(...)``  — derives cost_of_debt, corporate_tax_rate,
       enterprise_value, and all price multiples.
    2. ``Ratios.build(...)``     — depends on ``valuation.enterprise_value``
       and ``valuation.corporate_tax_rate``, so must come second.
    """
