...
def build_scenario_summary_table(report: PEValuationReport) -> List[List[str]]:
    summary_table = []
    for scenario_name, r in report.scenarios.items():
        r: PEValuationResult = r
...
def build_eps_progression_table(report: PEValuationReport) -> List[List[str]]:
    eps_table = []
    for scenario_name, r in report.scenarios.items():
        r: PEValuationResult = r
        proj = r.eps_progression
...
def cli_print_valuation(metrics: StockMetrics, report: PEValuationReport) -> None:
    ...
    print(tabulate(
        summary_info, headers=["Metric", "Value"], tablefmt="fancy_grid",
        colalign=("left", "decimal"),
    ))
    print()

    print("\n-- Scenario Valuation Summary --")
    summary_headers = ["Scenario", "Intrinsic Value", "Future Value", "Total EPS Growth", "Status"]
