    ...
    def _check_growth_stage(self):
        ...
        if revenue_growth_rate > 0.2 and net_income_ttm < 0:
            self._add_factor(
                ...
                f"High revenue growth ({revenue_growth_rate*100:.2f}%) with negative net income, indicating a potentially high-risk, high-growth startup phase.",
...
def evaluate_dcf(stock_metrics: StockMetrics) -> ValuationCheckResult:
    checker = DCFChecker(stock_metrics)
