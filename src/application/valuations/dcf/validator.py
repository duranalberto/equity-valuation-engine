...
class DCFChecker(ValuationChecker):
    ...
    def _check_free_cash_flow(self):
        ...
        if fcf_ttm is None:
            self._add_factor(
                "Missing FCF (TTM)", "Free Cash Flow (TTM) data is missing.", FactorSeverity.CRITICAL
            )
        elif fcf_ttm < 0:
        ...
        if last_year_fcf is None:
            self._add_factor(
                "Missing Last Year FCF", "Last Year FCF data is missing.", FactorSeverity.CRITICAL
            )
        elif last_year_fcf < 0:
            self._add_factor(
                "Negative Last Year FCF",
                f"Last Year FCF is negative: {last_year_fcf}",
    ...
    def _check_profitability(self):
        ...
        if eps_ttm is None or eps_ttm <= 0:
            self._add_factor(
                "Negative EPS (TTM)",
                f"Earnings Per Share (TTM) is zero or negative: {eps_ttm}",
                FactorSeverity.CRITICAL,
                eps_ttm,
            )
        if net_income_ttm <= 0:
            self._add_factor(
                ...
                f"Net Income (TTM) is zero or negative: {net_income_ttm}",
                FactorSeverity.CRITICAL,
                net_income_ttm,
            )
        if operating_cf_ttm < 0:
            self._add_factor(
                ...
                f"Operating Cash Flow (TTM) is negative: {operating_cf_ttm}",
                FactorSeverity.CRITICAL,
                operating_cf_ttm,
            )
    ...
    def _check_risk_and_stability(self):
        ...
        if cost_of_debt is not None and cost_of_debt > 0.2:
            self._add_factor(
                "High Cost of Debt",
                f"Cost of Debt is very high (>{cost_of_debt:.1%}), increasing WACC risk.",
                FactorSeverity.CRITICAL,
                cost_of_debt,
            )
        ...
        if tax_rate is not None and tax_rate < 0:
            self._add_factor(
                "Negative Tax Rate",
                f"Corporate Tax Rate is negative: {tax_rate}",
                FactorSeverity.WARNING,
                tax_rate,
            )
        ...
        if beta is not None and beta > 2:
            self._add_factor(
                "High Beta",
                f"Stock Beta is high ({beta:.2f}), suggesting high volatility and uncertainty.",
                FactorSeverity.WARNING,
                beta,
            )
        ...
        if market_cap is not None and market_cap < 500_000_000:
            self._add_factor(
                "Small Market Cap",
                f"Market cap is small ($ {market_cap:,.0f}), may lead to less reliable long-term forecasts.",
                FactorSeverity.WARNING,
                market_cap,
            )
    ...
    def _check_growth_stage(self):
        ...
        if revenue_growth_rate is not None and revenue_growth_rate > 0.2 and net_income_ttm < 0:
            self._add_factor(
                ...
                f"High revenue growth ({revenue_growth_rate * 100:.2f}%) with negative net income, "
                "indicating a potentially high-risk, high-growth startup phase.",
                FactorSeverity.WARNING,
                revenue_growth_rate,
            )
