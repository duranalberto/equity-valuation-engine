...
class PEChecker(ValuationChecker):
    ...
    def _add_factor(self, name: str, message: str, severity: FactorSeverity, value: Optional[Union[float, int]] = None):
        weight = 0
        if severity == FactorSeverity.CRITICAL:
            weight = self.CRITICAL_WEIGHT
        elif severity == FactorSeverity.WARNING:
            weight = self.WARNING_WEIGHT
        
        self._factors.append(CheckFactor(
            name=name,
            message=message,
            severity=severity,
            weight=weight,
    ...
    def _check_earnings_stability(self):
        ...
        if eps_ttm is None or eps_ttm <= 0:
            self._add_factor("Negative/Missing EPS (TTM)", f"EPS (TTM) is negative or missing: {eps_ttm}. P/E valuation is invalid.", FactorSeverity.CRITICAL, eps_ttm)
        
        if pe_ttm is None:
             self._add_factor("Missing P/E (TTM)", "P/E (TTM) ratio is missing, cannot perform valuation.", FactorSeverity.CRITICAL)

        if pe_ttm is not None and pe_ttm > 40:
            self._add_factor("Very High P/E", f"P/E (TTM) is high: {pe_ttm:.1f}, indicating high market expectations and sensitivity to growth forecasts.", FactorSeverity.WARNING, pe_ttm)
    ...
    def _check_growth_metrics(self):
        ...
        if net_income_growth is None or net_income_growth <= 0:
            self._add_factor("Non-Positive Growth", f"Net Income Growth is zero or negative: {net_income_growth}. Growth-based P/E assumptions are compromised.", FactorSeverity.WARNING, net_income_growth)
        
        if peg_ratio is not None and peg_ratio < 0:
            self._add_factor("Negative PEG Ratio", f"PEG Ratio is negative: {peg_ratio:.2f}. Indicates negative earnings growth.", FactorSeverity.CRITICAL, peg_ratio)
        elif peg_ratio is not None and peg_ratio > 2.0:
             self._add_factor("High PEG Ratio", f"PEG Ratio is high: {peg_ratio:.2f}. May indicate the stock is significantly overvalued relative to its growth.", FactorSeverity.WARNING, peg_ratio)
    ...
    def _check_balance_sheet(self):
        ...
        if debt_to_equity is not None and debt_to_equity > 2.0:
            self._add_factor("High Debt-to-Equity", f"Debt-to-Equity ratio is high: {debt_to_equity:.2f}. High leverage increases risk to future EPS.", FactorSeverity.WARNING, debt_to_equity)
