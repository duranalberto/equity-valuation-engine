...
class MetricsLoader:
    ...
    def get_ttm_prev_value(self, field: FinancialField) -> Optional[float]:
        ...
        prev_annual = self.loader.get_annual_value(field, year_offset=1)
        return prev_annual
    ...
    def build_stock_metrics(self) -> sm.StockMetrics:
        ...
        return sm.StockMetrics(
            profile=model(sm.CompanyProfile),
            financials=model(sm.Financials),
            cash_flow=model(sm.CashFlow),
            balance_sheet=model(sm.BalanceSheet),
            ratios=None,
            market_data=model(sm.MarketData),
            valuation=self._build_valuation(),
            historical_data=self._build_historical_data(),
        )

    def build_model(self, model_cls: type) -> any:
